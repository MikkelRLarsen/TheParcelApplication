// <auto-generated />
using System;
using System.Collections.Generic;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using ParcelService.Infrastructure;

#nullable disable

namespace ParcelService.Infrastructure.Migrations
{
    [DbContext(typeof(EFAppContext))]
    [Migration("20260428090253_AddedRegion")]
    partial class AddedRegion
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.4")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("ParcelService.Domain.Entities.Parcel", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid");

                    b.Property<int>("Priority")
                        .HasColumnType("integer");

                    b.Property<decimal>("Weight")
                        .HasColumnType("numeric");

                    b.ComplexProperty(typeof(Dictionary<string, object>), "Receiver", "ParcelService.Domain.Entities.Parcel.Receiver#Party", b1 =>
                        {
                            b1.IsRequired();

                            b1.ComplexProperty(typeof(Dictionary<string, object>), "PersonalInformation", "ParcelService.Domain.Entities.Parcel.Receiver#Party.PersonalInformation#PersonInfo", b2 =>
                                {
                                    b2.IsRequired();

                                    b2.Property<string>("Name")
                                        .IsRequired()
                                        .HasColumnType("text");

                                    b2.ComplexProperty(typeof(Dictionary<string, object>), "Address", "ParcelService.Domain.Entities.Parcel.Receiver#Party.PersonalInformation#PersonInfo.Address#Address", b3 =>
                                        {
                                            b3.IsRequired();

                                            b3.Property<string>("City")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("Country")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("HouseNumber")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("Street")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("ZipCode")
                                                .IsRequired()
                                                .HasColumnType("text");
                                        });
                                });

                            b1.ComplexProperty(typeof(Dictionary<string, object>), "Terminal", "ParcelService.Domain.Entities.Parcel.Receiver#Party.Terminal#Terminal", b2 =>
                                {
                                    b2.IsRequired();

                                    b2.Property<Guid>("Id")
                                        .HasColumnType("uuid");

                                    b2.ComplexProperty(typeof(Dictionary<string, object>), "Region", "ParcelService.Domain.Entities.Parcel.Receiver#Party.Terminal#Terminal.Region#Region", b3 =>
                                        {
                                            b3.IsRequired();

                                            b3.Property<string>("Country")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("MainRegion")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("SubRegion")
                                                .IsRequired()
                                                .HasColumnType("text");
                                        });
                                });
                        });

                    b.ComplexProperty(typeof(Dictionary<string, object>), "Sender", "ParcelService.Domain.Entities.Parcel.Sender#Party", b1 =>
                        {
                            b1.IsRequired();

                            b1.ComplexProperty(typeof(Dictionary<string, object>), "PersonalInformation", "ParcelService.Domain.Entities.Parcel.Sender#Party.PersonalInformation#PersonInfo", b2 =>
                                {
                                    b2.IsRequired();

                                    b2.Property<string>("Name")
                                        .IsRequired()
                                        .HasColumnType("text");

                                    b2.ComplexProperty(typeof(Dictionary<string, object>), "Address", "ParcelService.Domain.Entities.Parcel.Sender#Party.PersonalInformation#PersonInfo.Address#Address", b3 =>
                                        {
                                            b3.IsRequired();

                                            b3.Property<string>("City")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("Country")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("HouseNumber")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("Street")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("ZipCode")
                                                .IsRequired()
                                                .HasColumnType("text");
                                        });
                                });

                            b1.ComplexProperty(typeof(Dictionary<string, object>), "Terminal", "ParcelService.Domain.Entities.Parcel.Sender#Party.Terminal#Terminal", b2 =>
                                {
                                    b2.IsRequired();

                                    b2.Property<Guid>("Id")
                                        .HasColumnType("uuid");

                                    b2.ComplexProperty(typeof(Dictionary<string, object>), "Region", "ParcelService.Domain.Entities.Parcel.Sender#Party.Terminal#Terminal.Region#Region", b3 =>
                                        {
                                            b3.IsRequired();

                                            b3.Property<string>("Country")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("MainRegion")
                                                .IsRequired()
                                                .HasColumnType("text");

                                            b3.Property<string>("SubRegion")
                                                .IsRequired()
                                                .HasColumnType("text");
                                        });
                                });
                        });

                    b.ComplexProperty(typeof(Dictionary<string, object>), "Tracking", "ParcelService.Domain.Entities.Parcel.Tracking#Tracking", b1 =>
                        {
                            b1.IsRequired();

                            b1.Property<string>("Status")
                                .IsRequired()
                                .HasColumnType("text");

                            b1.Property<Guid>("TrackingNumber")
                                .HasColumnType("uuid");
                        });

                    b.HasKey("Id");

                    b.ToTable("Parcel", (string)null);
                });
#pragma warning restore 612, 618
        }
    }
}
